...
class EditModal(discord.ui.Modal):
    ...
    async def on_submit(self, interaction: discord.Interaction):
        value = self.text_input.value if self.text_input.value else f"No {self.field.replace('_', ' ')} provided"
...
def create_review_embed(review: dict):
    """Create a formatted embed for a review. Returns (embed, file_or_none)"""
    ...
    # Image handling - stored base64 first, fallback to URL
    image_file = None
